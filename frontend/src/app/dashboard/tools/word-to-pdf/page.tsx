...
export default function WordToPDFPage() {
  ...
  const wordToPDFMutation = useMutation({
    mutationFn: async (formData: FormData) => {
      const response = await api.post("/tools/pdf/word-to-pdf", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      return response.data;
    },
    onSuccess: (data) => {
      setStatus("success", "Word document converted to PDF!");
      setResult(data);
  ...
  const handleConvert = () => {
    if (!selectedFiles.length) {
      toast.error("Please select a Word file");
      return;
    }
  ...
  return (
    <div className="max-w-6xl mx-auto">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold flex items-center gap-3">
              <div className="p-2 rounded-xl bg-gradient-to-br from-violet-500/10 to-purple-500/10">
                <FileType className="h-7 w-7 sm:h-8 sm:w-8 text-violet-500" />
              </div>
              Word to PDF
            </h1>
          ...
          <Card className="overflow-hidden">
            <CardHeader className="bg-gradient-to-r from-violet-500/10 to-purple-500/10">
              <CardTitle className="flex items-center gap-2">
                <FileType className="h-5 w-5 text-violet-500" />
                Upload Word Document
              </CardTitle>
              <CardDescription>
                Drag and drop or click to select your .docx file
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/50">
                <FileType className="h-5 w-5 text-violet-500 flex-shrink-0" />
                <p className="text-sm text-muted-foreground">
                  Your Word document will be converted to PDF with preserved formatting and layout.
                </p>
              </div>
              ...
              <div className="pt-4">
                <Button
                  className="w-full h-12 text-base font-semibold bg-gradient-to-r from-violet-500 to-purple-500"
                  onClick={handleConvert}
                  disabled={!selectedFiles.length || wordToPDFMutation.isPending}
                >
                  {wordToPDFMutation.isPending ? (
                    <>
            ...
            <CardContent>
              {wordToPDFMutation.isPending ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Upload a Word document to convert
                  </p>
      ...
      <RelatedGuide guideSlug="how-to-convert-word-to-pdf" />
    </div>
  );
}
